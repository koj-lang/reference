# Summary

[Introduction](./introduction.md)

# The language

- [Lexical Structure](lexical_structure/index.md)
  - [Input Format](lexical_structure/input_format.md)
  - [Whitespace](lexical_structure/whitespace.md)
  - [Comments](lexical_structure/comments.md)
  - [Keywords](lexical_structure/keywords.md)
  - [Identifiers](lexical_structure/identifiers.md)
  - [Literals](lexical_structure/literals.md)
  - [Punctuation](lexical_structure/punctuation.md)
  - [Tokens](lexical_structure/tokens.md)
- [Statements and Expressions](statements-expressions.md)
  - [Statements](statements/index.md)
    - [Type Definitions](statements/type_defs.md)
    - [Declarations](statements/declarations.md)
  - [Expressions](expressions/index.md)
    - [Literal Expressions](expressions/lit_expr.md)
    - [Parenthesized Expressions](expressions/parens_expr.md)
    - [Block Expressions](expressions/block_expr.md)
    - [Operator Expressions](expressions/operator_expr.md)
- [Type System](type_system/index.md)
  - [Primitive Types](type_system/primitives.md)
  - [Array Types](type_system/array.md)
  - [Tuple Types](type_system/tuple.md)
  - [Function Types](type_system/function.md)
  - [Struct Types](type_system/struct.md)
  - [Enum Types](type_system/enum.md)
  - [Inferred Type](type_system/inferred.md)
